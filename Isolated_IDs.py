...
def mask_extraction():
    ...
    for key, val in sorted_filepaths.items():
        if arcpy.Exists(val):
            ...
            count += 1
            if count % 5 == 0:
                print(f"Mask Extraction: {filtered_layerA}.")
...
def working_copy():
    sr = arcpy.SpatialReference(4326)
    db = os.path.join(project_folder, "Default.gdb")
    output_excel_path_ip = os.path.join(project_folder, "NIP_PointLayer.xlsx")
...
def true_value(new_key):
    ...
    for index, row in df.iterrows():
        ...
        dist_to_central = ep.distanceTo(sp)
        ...
        if filename.endswith('.shp'):
            ...
            if int(key) in num_arr:
                ...
                with arcpy.da.InsertCursor(line_fin, ["SHAPE@", "FID"]) as cursor:
                    for key_temp, val in data_dict.items():
                        try:
